...
from .projectCourseAlignmentServices import getProjectCourseAlignmentsByCourseID,addProjectCourseAlignments,deleteProjectCourseAlignments
...
def getAllCoursesOfProject(project_id):
    try:
        ...
        course_data = session.query(ProjectCourse).filter(ProjectCourse.project_id == project_id).all()
...
def getProjectCourseByID(project_id,course_id):
    try:    
        ...
        course_data = session.query(ProjectCourse) \
            .filter(condition) \
            .first()
        closeSession(session)
...
def updateProjectCourseByID(project_course, alignments):
    try:
        ...
        print(addProjectCourseAlignments(alignments, existing_project_course.id, session))
        session.commit()
        closeSession(session)
...
def deleteProjectCourse(project_course_id,project_id):
    try:
        ...
        session.query(ProjectProgramCourseXref) \
            .filter_by(project_id=project_id, course_id=project_course_id).delete(synchronize_session=False)
...
def searchProjectCourse(search_query):
    try:
        ...
        search_results = session.query(Course).filter(
            or_(
        ...
        return jsonify({"error": "Failed to search Project Course", "message": str(e)}), 500
...
def copyCoursesToProject(project_id, course_ids):
    try:
        ...
        for course_id in course_ids:
            ...
            alignments = session.query(CourseAlignment).filter_by(course_id=course_id).all()
        ...
        session.rollback()
        closeSession(session)
        return jsonify({"error": "Failed to copy courses to project", "message": str(e)}), 500
...
def beginRevision(project_id, course_id):
    try:
        ...
        project_course = session.query(ProjectCourse).filter_by(project_id=project_id, id=course_id).first()
