...
from app.services.courseServices import getAllCoursesOfProject,getProjectCourseByID,addProjectCourse,updateProjectCourseByID,deleteProjectCourse,searchProjectCourse,copyCoursesToProject,beginRevision
...
def deleteProjectProgram_route():
    ...
    try:
        ...
        if project_program_id is None or project_id is None:
            return jsonify({'error': 'Missing project_program_id or project_id parameter'}), 400
...
def add_project():
    if not validate_login():
        ...
        if any(value is None for value in [pname, owner, default_read, default_read_write]):
            return jsonify({'message': 'Missing required parameters'}), 400
...
def updateproject():
    if not validate_login():
        ...
        if any(value is None for value in [id, name, owner, default_read_write, default_read, ]):
            return jsonify({'message': 'Missing required parameters'}), 400
